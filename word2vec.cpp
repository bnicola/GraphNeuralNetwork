...
static constexpr int    WINDOW_SIZE = 2;     // context window (each side)
static constexpr double LR          = 0.001; // Adam learning rate
static constexpr int    EPOCHS      = 5;     // passes over the corpus
static constexpr int    BATCH_SIZE  = 32;    // samples per weight update
static constexpr int    MIN_FREQ    = 1;     // drop words rarer than this
static constexpr int    PRINT_EVERY = 500;   // batches between progress lines
...
int main(int argc, char** argv)
{
    ...
    int step = std::max(1, (V - 2) / 6);
    for (int v = 2; v < V && (int)probes.size() < 6; v += step)
        probes.push_back(vocab.word(v));

    for (auto& word : probes)
    {
        std::cout << "\n  '" << word << "':\n";
        auto neighbors = nearest_neighbors(word, vocab, embeddings, 5);
        for (auto& [sim, w] : neighbors)
